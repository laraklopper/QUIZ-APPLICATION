...
export default function Page2(//Export default page2 function component
  ...
  useEffect(() => {
    ...
    const loadQuizzes = async () => {//Define an async function to loadQuizzes
      try {
...
//==========================================
  ...
  const fetchQuiz = useCallback(async (quizId) => {//Define async function to fetch a single Quiz
    ...
    //Conditional rendering to check if a quiz is selected
      ...
      const response = await fetch(`${config.API_BASE_URL}/quiz/findQuiz/${quizId}`
        /*`http://localhost:3001/quiz/findQuiz/${quizId}`*/, {
  ...
  return (
    <>
      ...
      <Header heading="GAME" />
      {/* Section1: quizdisplay component and 
      ...
      <section className='section1'>
       ...
       />
        <div>          
           {/*Render the Quiz Display component */}
          <QuizDisplay
            selectedQuizId={selectedQuizId}
            quiz={quiz}
            fetchQuiz={fetchQuiz}
