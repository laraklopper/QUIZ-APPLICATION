...
//CSS stylesheets
import '../CSS/Page4.css';//Import Page4 stylesheet
//Bootstrap
...
export default function Page4({//Export default Page4 function component
  ...
  const editUser = async (userId) => {//Define an async funcion to edit User details
    try {
      ...
      const response = await fetch(`${config.API_BASE_URL}/users/editAccount/${userId}`
        /*`http://localhost:3001/users/editAccount/${userId}`*/, {
  ...
  const deleteUser = async (userId) => {//Define an async function to delete a user
    try {
      ...
      const response = await fetch(`${config.API_BASE_URL}/users/deleteUser/${userId}`
