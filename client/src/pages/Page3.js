...
import '../CSS/quizList.css'//Import quizList stylesheet
// Bootstrap
...
export default function Page3(//Export default Page3 function component 
 ...
 or when fetchQuizzes function changes*/
  ...
  const addNewQuiz = useCallback(async () => {//Define an async function to add a new quiz
    ...
    try {
      ...
      const response = await fetch(`${config.API_BASE_URL}/quiz/addQuiz`
        /*'http://localhost:3001/quiz/addQuiz'*/, {
  ...
  const editQuiz = useCallback(async (quizId) => {//Define an async function to edit a quiz
    try {
      ...
      //Send a PUT request to the backend URL  to edit a quiz
      const response = await fetch(`${config.API_BASE_URL}/quiz/editQuiz/${quizId}`
       /* `http://localhost:3001/quiz/editQuiz/${quizId}`*/, {
...
//Function to delete a quiz
  const deleteQuiz = useCallback (async (quizId) => {
    try {
      ...
      const response = await fetch(`${config.API_BASE_URL}/quiz/deleteQuiz/${quizId}`
        /* `http://localhost:3001/quiz/deleteQuiz/${quizId}`*/, {
