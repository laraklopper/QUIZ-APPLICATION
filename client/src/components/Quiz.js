...
//CSS stylesheets
import '../CSS/quiz.css';//Import the quiz stylesheet 
// Bootstrap
...
export default function Quiz({//Export default Quiz function component
  ...
  // =============STATE VARIABLES=====================
  const [selectedOption, setSelectedOption] = useState(null);// State used to track the selected answer option 
  const [feedback, setFeedback] = useState('');// State used to show feedback message for correct/incorrect answers              
  const [timeLeft, setTimeLeft] = useState(10);// State to track the time left for the timer

  // ========USE EFFECT HOOK==================
  /* Effect to reset the question index to the first 
  question when the questions array changes */
  useEffect(() => {
    //Condititional rendering to ensure the questions array is not emptey
    if (questions.length > 0) {
      // console.log('Questions loaded. Resetting quiz index to 0.');//Load a message in the console for debugging purposes
      setQuizIndex(0);// Reset the quiz index to the first question
    }
    ...
    <div id='quiz'>
      ...
      <div>
        ...
        <Row className='quizNavRow'>
          <Col xs={6} md={4} className='resultCol'>
            <div>
          ...
          <Col xs={6} md={4}>
            ...
            <Button
              ...
              type='button'
              id='nextBtn'
              onClick={handleNextQuestion}//Call the handle next question function
            ...
            <Button
              variant='primary'
              type='reset'
              onClick={handleRestart}//Call the restart quiz function
              id='resetBtn'
