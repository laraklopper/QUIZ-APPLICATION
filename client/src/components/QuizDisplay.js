...
export default function QuizDisplay(//Export default QuizDisplay function component
  ...
  const [currentScore, setCurrentScore] = useState(0);// State to store the user's score during the quiz
    ...
    const checkExistingScore = useCallback(async () => {
      try {
        ...
        const response = await fetch(`${config.API_BASE_URL}/scores/findQuizScores/${quizName}/${currentUser.username}`
         /* `http://localhost:3001/scores/findQuizScores/${quizName}/${currentUser.username}`*/, {
  ...
  const updateScore = useCallback(async (scoreId) => {
    ...
    try {      
      ...
      const response = await fetch(`${config.API_BASE_URL}/scores/updateScore/${scoreId}`
        /* `http://localhost:3001/scores/updateScore/${scoreId}`*/, {
  ...
  const addScore = useCallback(async () => {//Define an async function to add a users Score
    try {
      ...
      const response = await fetch(`${config.API_BASE_URL}/scores/addScore`
        /*`http://localhost:3001/scores/addScore`*/, {
