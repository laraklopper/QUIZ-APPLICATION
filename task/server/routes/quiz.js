...
router.get('/findQuiz/:id', checkJwtToken, async (req, res) => {
    try {
        ...
        // Conditional rendering to check if the provided ID is a valid MongoDB ObjectId
...
router.put('/editQuiz/:id', checkJwtToken,  async (req, res) => {
    ...
    try {
        ...
        console.log(updatedQuiz); // Log the updated quiz for debugging purposes
...
router.delete('/deleteQuiz/:id', checkJwtToken,async (req, res) => {
    ...
    try {
        ...
        res.status(200).json({ message: 'Quiz successfully deleted' });// Respond with a 200 OK status and a success message
    } 
    catch (error) {
        //Error handling
