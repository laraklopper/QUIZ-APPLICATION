...
const authenticateToken = (req, res, next) => {
    ...
    if (token == null) return res.sendStatus(401);// If there's no token, respond with 401 (Unauthorised) response status code in the JSON response
    ...
    jwt.verify(token, 
        ...
        (err, user) => {
            if (err) return res.sendStatus(403);// If the token is invalid, respond with 401 (Unauthorised) response status code in the JSON response
...
const editAuthorization = async (req, res, next) => {
    ...
    try {
        ...
        if (quiz.username === req.user.username || req.user.admin) {
            ...
            res.status(403).json({message: 'Unauthorised to edit this quiz'})
        }
    } catch (error) {
...
const deleteAuthorization = async (req, res, next) => {
    ...
    try {
        ...
        if (quiz.username === req.user.username || req.user.admin) {
            next()//If the user is authorised to delete the quiz move to the next middleware function or route handler
        } else {
