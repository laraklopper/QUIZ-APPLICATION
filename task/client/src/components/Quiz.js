...
export default function Quiz({
  ...
  useEffect(() => {
    ...
    return () => {
      isMounted = false// Set isMounted to false to prevent state updates after unmount
      clearInterval(interval);// Clear the interval to prevent memory leaks
    }
  }, [timer, handleNextQuestion, quizTimer]);
  // Dependencies array: the effect runs when timer, handleNextQuestion, or quizTimer changes
