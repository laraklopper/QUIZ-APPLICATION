    ...
    setQuizList(//Update the quiz list (`quizList`) by mapping over it
      quizList.map(q =>
        ...
        q._id === quiz._id
          ...
          : q // Otherwise, return the quiz unchanged
      )
    );
  };
        ...
        <Col xs={6} md={4} className='editQuizCol'>
          <label className='editQuizLabel'>
            ...
            <input
              ...
              name='newQuizName'// Sets the name attribute for form handling
              value={newQuizName}// Binds the input value to the newQuizName state variable
              // Update the newQuizName state with the input value
              onChange={(e) => setNewQuizName(e.target.value)}
              autoComplete='off'// Disabler autocomplete
